...
function SkyBison(initcmdline)
	...
	" Use try/catch to make sure we always properly clean up. Note that when
	" the cmdline is eventually run, it should be outside of this try/catch so
	" any errors generated by the user-supplied cmdline are presented to the
	" user.
	try
	...
	endwhile
		...
		" Despite the error occurring, we should still clean up.
		return s:RunCommandAndQuit("")
	endtry
	" The while loop above broke because some condition occurred to run the
	" cmdline contents (such as <cr>).  Running it here instead of in the
	" while loop so it is outside the try/catch, so if it makes an error that
	" error is properly displayed to the user.  Also, apparently netrw relies
	" on this not being in a try/catch.
